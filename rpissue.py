...
import gspread 
# Lưu ý: Import st.secrets.get() không cần khai báo riêng, nó là một phần của st
...
# SERVICE_ACCOUNT_FILE chỉ dùng khi chạy LOCAL, khi DEPLOY sẽ dùng st.secrets
...
# --- CÁC HÀM XỬ LÝ API VÀ GSHEET (ĐÃ CẬP NHẬT CHỨC NĂNG BẢO MẬT) ---

def get_google_sheet_worksheet_by_id(spreadsheet_id, worksheet_name):
    """
    Kết nối tới Google Sheet bằng Service Account.
    Ưu tiên đọc thông tin từ Streamlit Secrets ('gcp_service_account') khi được triển khai.
    """
    try:
        # 1. Thử đọc từ Streamlit Secrets (Ưu tiên khi Deploy)
        secrets = st.secrets.get("gcp_service_account")
        
        if secrets:
            # Nếu tìm thấy secrets, sử dụng nó để tạo Credentials
            creds = ServiceAccountCredentials.from_service_account_info(
                secrets,
                scopes=['https://www.googleapis.com/auth/spreadsheets']
            )
            # st.success("✅ Đã kết nối Google Sheet bằng Streamlit Secrets (Deployed Mode).") # Có thể ẩn thông báo này khi deploy chính thức
        else:
            # 2. Quay lại đọc từ file (Chỉ dành cho chạy cục bộ)
            # st.warning(f"⚠️ Không tìm thấy Streamlit Secrets. Thử kết nối bằng file: {SERVICE_ACCOUNT_FILE}") # Có thể ẩn thông báo này khi deploy chính thức
        ...
        st.error(f"❌ Lỗi kết nối/xác thực Google Sheet: {e}")
        return None
...
def write_to_google_sheet(data_row: Dict[str, Any]):
    ...
    try:
        # Lấy số dòng hiện tại trong cột B để xác định dòng tiếp theo
        ...
        # Ghi các cột liên tục từ B đến G
        worksheet.update(f'B{next_row}', [data_main])
        # Ghi cột I (Issue) riêng
...
def load_headers_from_sheet():
    ...
    try:
        ...
        if not worksheet:
            # Lỗi đã được xử lý trong hàm get_google_sheet_worksheet_by_id
            return None
...
def fetch_operator_by_status(headers, sub_pickup_id, status_to_find=2):
    """Call API 2 để lấy operator tương ứng với 'status': 2 (Picker cuối cùng)."""
    if not headers:
        return None
    full_sub_pickup_id = f"{sub_pickup_id}_0"
    encoded_sub_pickup_id = urllib.parse.quote(full_sub_pickup_id)
    ...
    try:
        ...
        if data.get("retcode") == 0 and data.get("data") and data["data"].get("list"):
            ...
            # Duyệt ngược để tìm log status 2 gần nhất (thường là người cuối cùng hoàn thành)
            for log_item in reversed(tracking_list): 
            ...
            return {"message": f"Không tìm thấy log có status: **{status_to_find}**.", "operator": "N/A", "ctime": "N/A"}
        else:
            return {"message": f"Lỗi API 2 hoặc không có dữ liệu: {data.get('message', 'Lỗi không rõ')}", "operator": "N/A", "ctime": "N/A"}
    except requests.exceptions.RequestException as e:
        return {"message": f"Lỗi kết nối API 2: {e}", "operator": "N/A", "ctime": "N/A"}
    except Exception as e:
        return {"message": f"Lỗi xử lý JSON/dữ liệu API 2: {e}", "operator": "N/A", "ctime": "N/A"}
...
def run_streamlit_app():
    ...
    with st.form(key='wms_form'):
        ...
        tag_input_email = st.selectbox("Acc Tag:", options=MENTION_OPTIONS_EMAIL, index=len(MENTION_OPTIONS_EMAIL)-1) # Mặc định chọn @all
    ...
    if submitted:
        ...
        if not headers:
            results_container.error("❌ Lỗi: Không thể tải Cookie. Vui lòng kiểm tra Sheet Cookie và Service Account.")
            return
